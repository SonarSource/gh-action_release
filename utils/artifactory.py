...
import urllib
import requests
from main import artifactory_apikey
...
class Artifactory:
    ...
    def promote(self, release_request, buildinfo):
        ...
        if buildinfo.is_multi():
            ...
            url = f"{artifactory_url}/api/plugins/execute/multiRepoPromote?params=" + ";".join("{!s}={!r}".format(key, val) for (key, val) in params.items())
            r = requests.get(url, headers=self.headers)
        else:
