...
def _download_parent_poms(artifactory, dest_dir, repo, gid, aid, version, patterns, seen):
    """Walk up a just-downloaded artifact's <parent> chain, downloading each ancestor's own pom.

    Central's bundle validator reads groupId/artifactId/version/licenses/scm/developers directly
    from each POM's own XML and does not resolve <parent> inheritance, so a consumer resolving
    this artifact's parent needs that pom to exist on Central too - an aggregator/parent pom is
    never listed in artifactsToPublish itself (it has no jar to publish) but still deploys its own
    pom, so it is reachable this way. seen is shared across every artifact of the same build, so a
    common aggregator is only downloaded once. The walk stops, with a warning, as soon as an
    ancestor is non-public, excluded, or missing from repo (e.g. an externally-published,
    already-on-Central common parent) - either way there is nothing further to bundle from here.
    """
    while True:
        pom_path = _pom_path(dest_dir, gid, aid, version)
        parent = _read_parent_gav(pom_path)
        if not parent:
            return
        gid, aid, version = parent
        if (gid, aid, version) in seen:
            return
        if not gid.startswith(PUBLIC_GROUP_ID_PREFIX):
            print(f"::warning::stopping parent walk at non-public {gid}:{aid}:{version}; "
                  "the published pom's <parent> will not resolve from Maven Central")
            return
        ancestor_path = f"{gid.replace('.', '/')}/{aid}/{version}/{aid}-{version}.pom"
        if any(fnmatch.fnmatchcase(aid, p) or fnmatch.fnmatchcase(ancestor_path, p) for p in patterns):
            print(f"::warning::stopping parent walk at excluded {gid}:{aid}:{version}")
            return
        seen.add((gid, aid, version))
        _download_optional(artifactory, dest_dir, repo, gid, aid, "", "pom", version)
        if os.path.exists(_pom_path(dest_dir, gid, aid, version)):
            print(f"bundling parent pom {gid}:{aid}:{version} for Maven Central")
        else:
            print(f"::warning::parent pom {gid}:{aid}:{version} not found in {repo}; "
                  "assuming it is already on Maven Central")
            return


def _pom_path(dest_dir, gid, aid, version):
    return os.path.join(dest_dir, gid.replace(".", "/"), aid, version, f"{aid}-{version}.pom")


def _read_parent_gav(pom_path):
    """Return the (groupId, artifactId, version) of pom_path's <parent>, or None if absent/unreadable."""
    try:
        root = ET.parse(pom_path).getroot()
    except (ET.ParseError, OSError) as e:
        print(f"could not read {pom_path} to find its <parent>: {e}")
        return None
    parent = next((child for child in root if _local_tag(child.tag) == "parent"), None)
    if parent is None:
        return None
    gav = {_local_tag(child.tag): (child.text or "").strip() for child in parent}
    gid, aid, version = gav.get("groupId"), gav.get("artifactId"), gav.get("version")
    return (gid, aid, version) if gid and aid and version else None
