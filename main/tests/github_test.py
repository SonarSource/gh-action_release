...
@patch.dict(os.environ, {
    ...
    'GITHUB_SHA': 'sha',
    'ARTIFACTORY_BUILD_NAME': 'other-org-build',
}, clear=True)
@patch('release.utils.github.json.load',
       return_value={
           'repository': {
...
def test_artifactory_build_name_env_overrides_repo_name(mock_release_event):
    with patch('release.utils.github.open', mock_open()) as open_mock:
        release_request = GitHub().get_release_request()
        assert release_request.org == 'other-org'
        assert release_request.project == 'project'
        assert release_request.artifactory_build_name == 'other-org-build'
        open_mock.assert_called_once()
        mock_release_event.assert_called_once()
